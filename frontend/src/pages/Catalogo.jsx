...
const EMPTY_PRODUCTO = { nombre: '', precio: '', categoria: '', activo: true, imagen: '' };
const EMPTY_CONFIG = { clave: '', valor: '', descripcion: '' };
...
export default function Catalogo() {
  ...
  const [formP, setFormP] = useState(EMPTY_PRODUCTO);
  ...
  /* ── Productos ── */
  const openNewP = () => { setFormP(EMPTY_PRODUCTO); setEditP({}); setError(''); };
  const openEditP = (p) => {
    setFormP({
      nombre: p.nombre, precio: p.precio,
      categoria: p.categoria || '', activo: p.activo, imagen: p.imagen || '',
    });
    setEditP(p);
    setError('');
  };
  ...
  const saveP = async () => {
    if (!formP.nombre.trim() || !formP.precio) {
  ...
  /* ── Configs ── */
  const openNewC = () => { setFormC(EMPTY_CONFIG); setEditC({}); setError(''); };
  const openEditC = (c) => {
    setFormC({ clave: c.clave, valor: c.valor, descripcion: c.descripcion });
    setEditC(c);
    setError('');
  };
  ...
  const saveC = async () => {
    if (!formC.clave.trim() || !formC.valor.trim()) {
      ...
      <main className="sidebar-main">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {prods.map((p) => (
                    <div key={p.id} className="wf-box" style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                      ...
                      <span className="wf-h3" style={{ color: 'var(--sj-green-d)', flexShrink: 0 }}>{fmtMoney(p.precio)}</span>
        ...
        {seccion === 'temas' && (
          <>
            ...
            <div className="wf-sm" style={{ marginBottom: 16 }}>
              Cambia la paleta de colores del sistema. El cambio se aplica al instante.
            </div>
